...
def test_least_time_with_walk():
    ...
    scenario = {
        ...
        "goal": "least-time",
        "walk_edges": [
            {"from": "AEON_MALL_2", "to": "THE_BREEZE", "minutes": 8},
            {"from": "THE_BREEZE", "to": "AEON_MALL_2", "minutes": 8},
        ],
    }
    res = route.solve_scenario(scenario, rs, st, tr)
    assert "error" not in res
...
def test_monotone():
    ...
    for tid, lst in by_trip.items():
        times = [
            route.time_to_min(x["arrival_time"]) for x in sorted(lst, key=lambda z: z["stop_seq"])
        ]
