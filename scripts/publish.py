...
def publish():
    ...
    manifest = {
        ...
        "endpoints": [p.name for p in sorted(DIST_DIR.glob("*.json"))] + (["routes.geojson"] if (DIST_DIR/"routes.geojson").exists() else []),
        "usage": {
    ...
    scen_li = '<li><a href="scenarios/index.json">scenarios/index.json</a> - scenarios results</li>' if scenarios_exist else ''
...
def main():
    ...
    if args.dry_run:
        ...
        meta = json.load(open(JSON_DIR / "build_meta.json")) if (JSON_DIR/"build_meta.json").exists() else {}
