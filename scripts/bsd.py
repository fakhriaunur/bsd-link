...
def do_scenario(scenario_path):
    ...
    print(f"  Order: {' -> '.join([res['origin']] + res['destinations_ordered'] + ([res['origin']] if res['return_to_origin'] else []))}")
...
def main():
    ...
    p.add_argument("--list-stops", action="store_true", help="list all stops with halte_index count")
    args = p.parse_args()
    ...
    if args.halte:
        ...
        for r in routes:
            print(f"{r['route_id']} {r['route_name']} {r['route_color_hex']} inferred={r['is_inferred']}")
